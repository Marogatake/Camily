$(function () {
  ...
  function NextSlider() {
    if (cancelFlag == 0) {
      ...
      var flowTimeGuide = 900,  textChangeTime = 600,
      ...
      element[2].animate({'left': -0.8*elementWidth[1]},0.85*elementFlowWidth[2]*flowTimeGuide/winWidth+0.15*elementFlowWidth[1]*flowTimeGuide/winWidth,'linear',
      function(){
  ...
  function PrevSlider(){
    if (cancelFlag == 0) {
      ...
      },0.85*elementFlowWidth[1]*flowTimeGuide/winWidth+0.15*elementFlowWidth[0]*flowTimeGuide/winWidth-textChangeTime*0.8);

      element[1].animate({'left': winWidth*0.5 - elementWidth[0]},0.85*elementFlowWidth[1]*flowTimeGuide/winWidth+0.15*elementFlowWidth[0]*flowTimeGuide/winWidth,'linear',
      function(){
        $(this).removeAttr('id').attr('id', 'product_center');
      });
      ...
      setTimeout(function(){
        cloneElement.animate({'left': -0.8*cloneWidth}, cloneFlowWidth*flowTimeGuide/winWidth,'linear',
        function(){
          $(this).removeAttr('id').attr('id', 'product_left');
        });
  ...
  function flowToNext(flowTimeGuide) {
    return function(){
      ...
      setTimeout(function(){
        if (elementQuantity <= 3) {
          // 商品数が３つのときに実行される
          cloneElement.animate({'left': winWidth*0.89},0.45*flowTimeGuide,'linear',
          function(){
            $(this).removeAttr('id').attr('id', 'product_right');
            defer.resolve();
          });
  ...
  function clickNumButton(clickNum) {
   if (cancelFlag == 0) {
     ...
     flowTimeGuide = 400,textChangeTime = 600;
     ...
     if (5 < numberDifference) {
       ...
       },flowTimeGuide*flowTimes-textChangeTime*0.68);
